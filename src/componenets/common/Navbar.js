...
import logo from "../../assets/Logo/Gemini.jfif"
...
const NavBar = ({ setProgress }) => {
    ...
    return (
        <div className={` flex sm:relative bg-black relative z-50 h-22 
        items-center justify-center border-b-[1px] border-b-richblack-700 translate-y- 
         transition-all duration-500 `}>
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                <Link to='/' onClick={() => { dispatch(setProgress(100)) }}>
                <img src={logo} width={80} alt="Study Notion" height={40}></img>

                </Link>
                
                <div className='flex '>
                    {/* <form onSubmit={handelSearch} className='lg:flex -mt-6  -mx-44 items-center  relative sm:hidden md:hidden'>
                                <input value={searchValue} onChange={(e) => { setSearchValue(e.target.value) }} id='searchinput' type="text" placeholder="Search" className=' absolute h-12 top-0 left-0 rounded-lg focus:ring-1 ring-richblack-400 w-96 px-2 py-1 text-[15px]  text-richblack-50 focus:outline-none  bg-richblack-700' />
                                <HiSearch type='submit' id='searchicon' size={20} className=" text-richblack-100 top-3 absolute cursor-pointer left-80 " />
                    </form> */}
                    <SearchBar></SearchBar>
                </div>
                ...
                <div className={`flex md:hidden -mx-20 relative  flex-row ${token !== null && user?.accountType !== "Instructor" ? " -left-12" : ""}`}>
                    <GiHamburgerMenu className={`w-16 h-8 fill-richblack-25 absolute left-6 -bottom-4 cursor-pointer ml-4 `} onClick={shownav} />
                    ...
                    <div ref={show} className='mobNav z-50 '>
                        <nav className='flex items-center flex-col absolute  max-w-[120px] -left-9 top-7 pt-2
                         bg-richblack-900
                         ' ref={show}>
                            {
                                token == null && (
                                    <Link to='/login' className='' onClick={() => { dispatch(setProgress(100)) }} >
                                        <button onClick={shownav} className='mt-4 scale-50 text-center
                            ...
                            }
                            {
                                token == null && (
                                    <Link to='/signup' className='text-yellow-50' onClick={() => { dispatch(setProgress(100)) }} >
                                        <button onClick={shownav} className=' text-center scale-50 text-[15px] px-5 py-2 
                                        rounded-md font-semibold bg-yellow-50 text-black  transition-all duration-200' >
                                            Signup
                                        </button>
                                    </Link>
...

                            {
                                token != null && (
                                    <div className=' mt-2' >
                                        <p className=' text-richblack-50 text-center mb-2 scale-75'>Account</p>
                                        
                                        <ProfileDropDown className="scale-50"/>
                                        
                                    </div>
                                )
                            }
                            <div className=' mt-4 mb-4 bg-richblack-25 w-[120px] h-[2px]'></div>
                            <p className='  text-yellow-50 font-semibold'>Courses</p>
                            <div className=' text-center start-3 '>
                                {
                                    sublinks?.length < 0 ? (<div></div>) : (
                                        sublinks?.map((element, index) => (
                                            <Link to={`/catalog/${element?.name}`} key={index} onClick={() => { dispatch(setProgress(30)); shownav() }} className="p-2 text-sm">
                                                <p className=' text-richblack-5 text-center scale-50'>
                                                    {element?.name}
                                                </p>
                                            </Link>
                                        )))
                            ...
                            <div className=' mt-4 mb-4 bg-richblack-25 w-[120px] h-[2px]'></div>
                            <Link to='/about' onClick={() => { dispatch(setProgress(100)); shownav() }}
                             className="p-2">
                                <p className=' text-richblack-5 scale-50 '>
                                    About
                                </p>
                            </Link>
                ...
                {/* Desktop Navbar */}
                <nav className=''>
                    <ul className=' flex-row gap-x-20 -mr-28 text-richblack-25  hidden md:flex'>
                        {
                                ...
                                <li key={index} >
                                    {
                                        element.title === "Catalog" ? (<div className=' flex items-center group relative cursor-pointer'>
                                            ...
                                            <div className='invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]'>
                                                <div className='absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-richblack-50'></div>
                                                {
                                                    sublinks?.length < 0 ? (<div></div>) : (
                                                        sublinks?.map((element, index) => (
                                            ...
                                            <Link to={"/signup"}>
                                                <div className="flex  items-center  ">
                                                    <p className='text-white'>Rent Out Your Space</p>
                                                </div>
